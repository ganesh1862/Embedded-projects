...
#define BOOT_LOADER_ADDR 0x00000000
...
int main(void) {
  ...
  while (1) {
    print_string("Application_1 is running Its an BLUE LED\n");
    DL_GPIO_setPins(GPIO_PORT, GPIO_BLUE_LED_PIN);
    delay_cycles(32000000);
    if (DEBUG_DATA_RECEIVE_FLAG) {
      if (is_app_valid(BOOT_LOADER_ADDR)) {
        print_string("Jumping to BOOT_LOADER\n");
        jump_to_app((uint32_t *)BOOT_LOADER_ADDR);
      }
      else {
        print_string("Boot loader NOt found\n");     
      }
    }
